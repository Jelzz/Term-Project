...
bool traverse(node* root, player p){
    ...
    while (root->feature != ""){
        ...
        // Check if the feature value is a string, which happens in "NaN" when the value is missing
        if (typeid(p.features[feature]) == typeid(string)){
            return false;
        }
        // otherwise, traverse the tree
        if (p.features[feature] > root->threshold){
            root = root->right;
        } else {
